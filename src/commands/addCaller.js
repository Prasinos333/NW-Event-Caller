...
import { MessageFlags, SlashCommandBuilder, VoiceChannel } from "discord.js";
...
async function execute(interaction) {
    ...
    try {
        ...
        if (textChannel instanceof VoiceChannel) {
            return interaction.reply({ content: 'Error: Cannot start in voice channel chat.', flags: MessageFlags.Ephemeral });
        }
        ...
        if (!voiceChannel.viewable) {
            return interaction.reply({ content: "Error: Lacking `View Channel` permission.", flags: MessageFlags.Ephemeral });
        }

        if (!voiceChannel.joinable) {
            return interaction.reply({ content: "Error: Lacking `Connect Channel` permission.", flags: MessageFlags.Ephemeral });
        }

        if (voiceChannel.full) {
            return interaction.reply({ content: "Error: Channel is full.", flags: MessageFlags.Ephemeral });
        }

        const callerType = interaction.options.getString('type');

        if (!callerType) {
            return interaction.reply({ content: 'Error: Unable to retrieve bot caller type.', flags: MessageFlags.Ephemeral });
        }
        ...
        if (hasBot) {
            return interaction.reply({ content: 'Error: Voice channel currently has active bot. Press stop to change type', flags: MessageFlags.Ephemeral });
        }
        ...
        if (!availableBot) {
            ...
            return interaction.reply({ content: 'Error: No available bots.', flags: MessageFlags.Ephemeral });
        }
        ...
        if (!hasTextPerms) {
            ...
            return interaction.reply({ content: `Error: \`${availableBot.client.user.username}\` doesn't have proper permissions for this text channel.`, flags: MessageFlags.Ephemeral });
            ...
            return interaction.reply({ content: `Error: \`${availableBot.client.user.username}\` doesn't have proper permissions for the voice channel.`, flags: MessageFlags.Ephemeral });
        } else {
